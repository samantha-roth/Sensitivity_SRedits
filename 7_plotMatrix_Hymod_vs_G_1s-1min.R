...
create_plot <- function(modeltime,model) {
  ...
  if(model=="Hymod"){
    ...
    df <- data.frame(
      ...
      SA_Time = c(
        Time_Sobol_Hymod[modeltime,]/3600,  
        Time_BASS_Hymod[modeltime,]/3600,  
        Time_Kriging_Hymod[modeltime,]/3600,  
        Time_AKMCS_Hymod[modeltime,]/3600
      )
    )
    title<- NULL
  }
...
# Display the plot
filename<- "New_Figures/plotMatrix_Hymod_vs_G_1s-1min.jpeg"
