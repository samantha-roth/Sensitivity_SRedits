...
create_plot <- function(modeltime,model) {
  ...
  if(model=="SACSMA"){
    ...
    df <- data.frame(
      ...
      SA_Time = c(
        Time_Sobol_SACSMA[modeltime,]/3600,  
        Time_BASS_SACSMA[modeltime,]/3600,  
        Time_Kriging_SACSMA[modeltime,]/3600,  
        Time_AKMCS_SACSMA[modeltime,]/3600
      )
    )
    title<- NULL
  }
  ...
  if(eval_time_lab[modeltime]=="1s") ylab<- "Sensitivity Analysis Time (h)"
  if(eval_time_lab[modeltime]=="10s") ylab<- NULL
  if(eval_time_lab[modeltime]=="1min") ylab<- NULL
...
SACSMA_plots <- list(
  create_plot(4,"SACSMA"),
  create_plot(5,"SACSMA"),
  create_plot(6,"SACSMA")
)
...
# Display the plot
filename<- "New_Figures/plotMatrix_SACSMA_vs_G_1s-1min.jpeg"
