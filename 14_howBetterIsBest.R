...
min_times<- rep(NA,length(whichSobolBest))
min_method<- rep(NA,length(whichSobolBest))

for(i in 1:length(whichSobolBest)){
  min_times[i]<- min(Mean_Time_BASS[i],Mean_Time_AKMCS[i],Mean_Time_Kriging[i])
  if(min_times[i]==Mean_Time_BASS[i]) min_method[i]<- "BASS"
  if(min_times[i]==Mean_Time_Kriging[i]) min_method[i]<- "Kriging"
  if(min_times[i]==Mean_Time_AKMCS[i]) min_method[i]<- "AKMCS"
}
