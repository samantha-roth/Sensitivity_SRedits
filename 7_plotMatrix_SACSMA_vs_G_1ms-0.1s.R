...
Time_Sobol_SACSMA<- Time_Sobol_arr[1,,]
Time_BASS_SACSMA<- Time_BASS_arr[1,,]
Time_Kriging_SACSMA<- Time_Kriging_arr[1,,]
Time_AKMCS_SACSMA<- Time_AKMCS_arr[1,,]
...
Time_Sobol_G<- Time_Sobol_arr[which(tested_D=="10D"),,]; rm(Time_Sobol_arr)
Time_BASS_G<- Time_BASS_arr[which(tested_D=="10D"),,]; rm(Time_BASS_arr)
Time_Kriging_G<- Time_Kriging_arr[which(tested_D=="10D"),,]; rm(Time_Kriging_arr)
Time_AKMCS_G<- Time_AKMCS_arr[which(tested_D=="10D"),,]; rm(Time_AKMCS_arr)

n_nodes<- dim(Time_AKMCS_SACSMA)[2]
n_modeltimes<- dim(Time_AKMCS_SACSMA)[1]
...
create_plot <- function(modeltime,model) {
  ...
  if(model=="SACSMA"){
    ...
    df <- data.frame(
      ...
      SA_Time = c(
        Time_Sobol_SACSMA[modeltime,]/60,  
        Time_BASS_SACSMA[modeltime,]/60,  
        Time_Kriging_SACSMA[modeltime,]/60,  
        Time_AKMCS_SACSMA[modeltime,]/60
      )
    )
    title<- NULL
    if(eval_time_lab[modeltime]=="1ms") ylab<- "Sensitivity Analysis Time (min)"
    if(eval_time_lab[modeltime]=="10ms") ylab<- NULL
    if(eval_time_lab[modeltime]=="0.1s") ylab<- NULL
  ...
  if(model=="G"){
    ...
    df <- data.frame(
      Method = rep(c("Sobol", "BASS", "Kriging", "AKMCS"), each = n_nodes),
      SA_Time = c(
        Time_Sobol_G[modeltime,]/3600,  
        Time_BASS_G[modeltime,]/3600,  
        Time_Kriging_G[modeltime,]/3600,  
        Time_AKMCS_G[modeltime,]/3600
      )
    )
    title<- eval_time_lab[modeltime]
    
    if(eval_time_lab[modeltime]=="1ms") ylab<- "Sensitivity Analysis Time (h)"
    if(eval_time_lab[modeltime]=="10ms") ylab<- NULL
    if(eval_time_lab[modeltime]=="0.1s") ylab<- NULL
...
SACSMA_plots <- list(
  create_plot(1,"SACSMA"),
  create_plot(2,"SACSMA"),
  create_plot(3,"SACSMA")
)
...
g <- grid.arrange(
  ...
  # Bottom row (SACSMA)
  textGrob("SACSMA", rot = 90, gp = gpar(fontsize = 22)),
  arrangeGrob(grobs = SACSMA_plots[1:3], ncol = 3),
...
# Display the plot
filename<- "New_Figures/plotMatrix_SACSMA_vs_G_1ms-0.1s.jpeg"
