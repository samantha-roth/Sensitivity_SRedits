...
for (i in 1:4) { #for tested_D = 20D or 30D, we need to add multiple times together
...
for (j in 1:length(tested_eval_time)) {
  Time_Sobol[i,j] <- T_Sobol + (tested_eval_time[j]-avg_eval_time)*Sobol_convergesize
  Time_Kriging[i,j] <- T_Kriging1 + T_Kriging2 + (tested_eval_time[j]-avg_eval_time)*Kriging_size+ T_KrigingSobol
  Time_AKMCS[i,j] <- T_AKMCS1 + T_AKMCS2 + T_AKMCS3 + T_AKMCS4 + T_AKMCS5 + T_AKMCS6+ 
    T_AKMCS7 + T_AKMCS8 + T_AKMCS9 + (tested_eval_time[j]-avg_eval_time)*AKMCS_size + T_AKMCSSobol
...
#An adjustment for AKMCS's time: for 30D, we were not able to get 
#an emulator with low enough RMSE in a reasonable amount of time
Time_AKMCS[6,9]<- NA
...
for (i in 1:length(tested_D)) {
  for (j in 1:length(tested_eval_time)){
    if(i==length(tested_D) & j== length(tested_eval_time)){
