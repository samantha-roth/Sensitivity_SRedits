...
create_plot <- function(modeltime,model) {
  ...
  if(model=="SACSMA"){
    ...
    df <- data.frame(
      ...
      SA_Time = c(
        Time_Sobol_SACSMA[modeltime,]/3600/24,  
        Time_BASS_SACSMA[modeltime,]/3600/24,  
        Time_Kriging_SACSMA[modeltime,]/3600/24,  
        Time_AKMCS_SACSMA[modeltime,]/3600/24
      )
    )
    title<- NULL
  }
...
SACSMA_plots <- list(
  create_plot(7,"SACSMA"),
  create_plot(8,"SACSMA"),
  create_plot(9,"SACSMA")
)
...
# Display the plot
filename<- "New_Figures/plotMatrix_SACSMA_vs_G_10min-10h.jpeg"
