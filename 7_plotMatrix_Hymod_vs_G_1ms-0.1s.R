...
create_plot <- function(modeltime,model) {
  ...
  if(model=="Hymod"){
    ...
    df <- data.frame(
      ...
      SA_Time = c(
        Time_Sobol_Hymod[modeltime,]/60,  
        Time_BASS_Hymod[modeltime,]/60,  
        Time_Kriging_Hymod[modeltime,]/60,  
        Time_AKMCS_Hymod[modeltime,]/60
      )
    )
    title<- NULL
...
# Display the plot
filename<- "New_Figures/plotMatrix_Hymod_vs_G_1ms-0.1s.jpeg"
