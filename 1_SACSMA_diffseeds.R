...
foreach(node = 1:4)%dopar%{ 
  ...
  folder <- paste0(folderpath,"SacSma/Sobol") 
  if (!dir.exists(folder)) dir.create(folder)
  folder <- paste0(folderpath,"SacSma/Sobol/seed",seed) 
  if (!dir.exists(folder)) dir.create(folder)
  ...
  for (m in 1:length(tot_size)){
    ...
    if (N >= 2){
      ...
      mat<- cbind(mat,known_pars[1],known_pars[2],known_pars[3])
      X <- split(t(mat), rep(1:dim(mat)[1], each = d+length(known_pars)))
