...
for(i in 1:nrow(Min_Time_Sobol)){
...
pdf(file = "./Sam_Figures/Figure_Timescale_MaxDiff_Poly.pdf",width = 12,height = 7)
par(mar=c(5,5,2.6,6))
...
load("/storage/group/pches/default/users/svr5482/Sensitivity_paper_revision/Ranking_Data/text_bestSeedMod")
...
pdf(file = "./Sam_Figures/Figure_Timescale_MaxDiff_UnifBest_Poly.pdf",width = 12,height = 7)
par(mar=c(5,5,2.6,6))
plot(Timescale_MaxDiff[nrow(Timescale_MaxDiff):1, ],breaks = brks,
     xlab="Time of single run",ylab="Number of input parameters",col=cols,
     cex.lab=1.5,cex.axis=1,main="")
