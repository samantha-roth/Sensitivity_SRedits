...
foreach(node = 1:4)%dopar%{ 
  ...
  folder<-paste0(folderpath,"Polynomial/",d,"D/BASS/seed",seed) #set folder depending on both d and node
  ...
  while (1>0) {
    ...
    start.time <- Sys.time()
    mod <- bass(X_BASS, Y, nmcmc = mcmc_size, nburn = 1e5, thin = 1e3,verbose = FALSE) # fit BASS model
    end.time <- Sys.time()
