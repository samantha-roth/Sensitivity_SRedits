...
#compare fastest on average for Sobol's G function to standard Sobol'
...
#load where Sobol' is best at 30D
load("./Ranking_Data/bestFor30D")
...
for(i in 1:nrow(textMat_SobolEnough)){
  for(j in 1:ncol(textMat_SobolEnough)){
    ...
    if(i==nrow(textMat_SobolEnough)){
      if(is.na(bestFor30D[j])) textMat_SobolEnough[i,j]<- NA
    }
...
for(i in 1:nrow(textMat_SobolMaxEnough)){
  for(j in 1:ncol(textMat_SobolMaxEnough)){
    ...
    if(i==nrow(textMat_SobolMaxEnough)){
      if(is.na(bestFor30D[j])) textMat_SobolMaxEnough[i,j]<- NA
    }
