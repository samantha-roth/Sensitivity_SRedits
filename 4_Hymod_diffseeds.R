...
foreach(node = 1:4)%dopar%{ 
  ...
  folder <- paste0(folderpath,"Hymod/AKMCS/seed",seed) 
  ...
  x_Hymod<- X_Hymod[indx, ]
  x_rest_Hymod <- X_Hymod[-indx, ]
  
  # Evaluate model outputs and fit a Kriging model
  start.time<- Sys.time()
  y <- apply(x_Hymod,1,Testmodel)      
  end.time<- Sys.time()
  ...
  U <- sqrt(a$MSE)
  print(paste0("max(U) =",max(U),", range = ",(max(a$Y_hat)-min(a$Y_hat))/20))
  ...
  if (max(U)>(max(a$Y_hat)-min(a$Y_hat))/20){
    while (1>0){
      ...
      x_add_Hymod <- x_rest_Hymod[m, ]
      x_rest_Hymod <- x_rest_Hymod[-m, ]
      
      # Evaluate the output of that sample and update
      start.time<- Sys.time()
      y_add <- apply(matrix(x_add_Hymod,ncol=length(x_add_Hymod),nrow=1),1,Testmodel)  
      end.time<- Sys.time()
      model_time<- difftime(end.time,start.time,units = "secs")
      T_model_AKMCS<- c(T_model_AKMCS,model_time)
      ...
      y <- append(y,y_add)
      x <- rbind(x,x_add)
      x_Hymod <- rbind(x_Hymod,x_add_Hymod)
      ...
      save(x_Hymod,file = paste0(folder,"/x_Hymod"))
