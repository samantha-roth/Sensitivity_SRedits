...
load(paste(folder, "/T_Kriging", sep=""))
load(paste(folder, "/T_pred_Kriging", sep=""))
load(paste(folder, "/Kriging_size_vec", sep=""))
load(paste(folder, "/Kriging_size", sep=""))
...
while (1>0){
  ...
  if (max(std)<=(max(a$Y_hat)-min(a$Y_hat))/20) save(a,file = paste(folder,"/a",sep=""))
